  ...
  implements ActionListener
{
    ...
    extends JPanel
  {
    ...
    public void addText(String text, String style)
    {
      try
      {
        this.document.insertString(this.document.getLength(), text, this.document.getStyle(style));
  ...
  public LogDetail()
  {
    ...
    this.buttonPanel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 20));
    ...
    this.packetTab.add(this.packetGeneral = new PairSection("General Information"));
    ...
    this.packetTab.add(this.packetType = new PairSection("Packet Type"));
    ...
    this.packetTab.add(this.packetDestination = new PairSection("Destination"));
    this.packetDestination.addRow("IP:");
    this.packetDestination.addRow("MAC:");
    this.packetDestination.addRow("Port:");
    
    this.packetTab.add(this.packetData = new PairSection("Packet Data"));
    ...
    this.tabbedPane.addTab("Payload Log Viewer", this.payloadTab = new JPanel());
    this.payloadTab.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
    this.payloadTab.setLayout(new BoxLayout(this.payloadTab, 3));
    
    this.payloadTab.add(this.payloadGeneral = new PairSection("General Information"));
    ...
    this.payloadTab.add(this.payloadDestination = new PairSection("Destination"));
    this.payloadDestination.addRow("IP:");
    this.payloadDestination.addRow("MAC:");
    this.payloadDestination.addRow("Port:");
    
    this.payloadTab.add(this.payloadData = new PairSection("Packet Data"));
    ...
    this.tabbedPane.addTab("Payload Data", this.paydataTab = new JPanel());
    this.paydataTab.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
    this.paydataTab.setLayout(new BoxLayout(this.paydataTab, 3));
    
    this.paydataTab.add(this.paydataHex = new TextSection("HEX"));
    this.paydataTab.add(this.paydataPosition = new PairSection("Position"));
  ...
  public void showPacket()
  {
    ...
    if (LogViewer.logModel.findColumn("Data") == -1)
    {
      ...
      if ((repeatCount.length() > 0) && (!repeatCount.equals("1")) && (endTime.length() > 0)) {
        this.packetGeneral.setValue(1, repeatCount + " times through " + toDate(endTime));
      } else {
      ...
      this.packetDestination.setValue(0, getColumnValue(array, "Destination IP"));
      this.packetDestination.setValue(1, getColumnValue(array, "Destination MAC"));
      this.packetDestination.setValue(2, getColumnValue(array, "Destination Port"));
    ...
    else
    {
      ...
      String endTime = getColumnValue(array, "End Time");
      if ((repeatCount.length() > 0) && (!repeatCount.equals("1")) && (endTime.length() > 0)) {
        this.payloadGeneral.setValue(1, repeatCount + " times through " + endTime);
      } else {
      ...
      this.payloadGeneral.setValue(4, getColumnValue(array, "Application Type"));
      ...
      this.payloadSource.setValue(2, getColumnValue(array, "Source Port"));
      
      this.payloadDestination.setValue(0, getColumnValue(array, "Destination IP"));
      this.payloadDestination.setValue(1, getColumnValue(array, "Destination MAC"));
      this.payloadDestination.setValue(2, getColumnValue(array, "Destination Port"));
      ...
      if (data.length() > 0)
      {
        ...
        int position = -1;
        String positionInBufferValue = getColumnValue(array, "Position in Buffer");
        ...
        this.paydataPosition.setValue(1, getColumnValue(array, "Position in Stream"));
        ...
        {
          this.paydataAscii.addText(toASCII(binary, 0, position), "normal");
          this.paydataAscii.addText(toASCII(binary, position, position + 1), "highlight");
          this.paydataAscii.addText(toASCII(binary, position + 1, binary.length), "normal");
        }
        else
        {
          this.paydataAscii.addText(toASCII(binary, 0, binary.length), "normal");
        }
        int r = 0;
        int offset = 0;
        while (offset < binary.length)
        {
          ...
          for (int c = 0; c < 16; c++) {
            if (offset + c < binary.length)
            {
              ...
              this.paydataHex.addText(" " + toHex(b), offset + c == position ? "highlight" : "normal");
            }
            else
            {
          ...
          for (int c = 0; c < 16; c++)
          {
            ...
            this.paydataHex.addText(new Character((char)b).toString(), offset + c == position ? "highlight" : "normal");
          }
  ...
  private String toHex(byte hexByte)
  {
    ...
    return new String(new char[] { hexDigits[(hexByte >> 4 & 0xF)], hexDigits[(hexByte & 0xF)] });
  }
  ...
  private static int hexnib(int b, char c)
    throws Exception
  {
    b <<= 4;
    int digitValue = c - '0';
  ...
  private static String toASCII(byte[] hexBytes, int start, int end)
  {
    ...
    for (int i = start; i < end; i++) {
      if (((hexBytes[i] < 32) || (hexBytes[i] > 126)) && (hexBytes[i] != 10) && (hexBytes[i] != 13)) {
        string.append('.');
      } else {
